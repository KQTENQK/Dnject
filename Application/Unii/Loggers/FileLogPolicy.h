#ifndef __UNII_FILELOGPOLICY__
#define __UNII_FILELOGPOLICY__
...
namespace Unii
{
    class FileLogPolicy : public ILogPolicy
    {
        ...
        public:
            FileLogPolicy(const std::string& filename) : m_filename(filename)
            {
                OpenFile();
            }
            ...
            void SetFile(const std::string& newFilename)
            {
                ...
                if (m_file.is_open())
                {
                    m_file << "=== Switching to file: " << newFilename 
